...
void app_draw(App *app) {
  XRenderColor black = {.red = UINT16_MAX * 0.09375,
  ...
  XRenderComposite(app->display, PictOpOver, app->source, 0, app->buffer, 0, 0, 0, 0, app->origin.x,
                   app->origin.y, app->size.x - app->origin.x, app->size.y - app->origin.y);
  ...
  if (app->focused) {
    ...
    XRenderFillRectangle(app->display, PictOpOver, app->buffer, &focus, 0, 0, start.x, app->size.y);

    XRenderFillRectangle(app->display, PictOpOver, app->buffer, &focus, end.x, 0,
                         app->size.x - start.x, app->size.y);

    XRenderFillRectangle(app->display, PictOpOver, app->buffer, &focus, start.x, 0, end.x, start.y);
